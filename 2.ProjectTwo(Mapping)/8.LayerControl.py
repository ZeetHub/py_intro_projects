...
for lt, ln, el in zip(lat,lon, elev):# when itering with two lists at the same time, use zip as indicated.
    fgv.add_child(folium.CircleMarker(location = [lt,ln], radius = 6, popup = str(el)+" m", tooltip = 'Hi there!',
...
map.add_child(folium.LayerControl())#Why not before the above line??
map.save("Map1.html")
