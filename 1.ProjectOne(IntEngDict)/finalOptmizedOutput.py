...
while(reply == 'y'):
    def keyVal(key):
        ...
        if key in data:
            ...
            rep = input("Did you mean %s instead? Press Y for yes and N for no: " % get_close_matches(key, data.keys())[0])
            ans = rep.upper()
            if ans == 'Y':
    ...
    if type(answer) == list:
        ...
        print(answer)
    reply = input("Do you want to continue? Press Y for yes and N for no: ")
    reply.lower()
